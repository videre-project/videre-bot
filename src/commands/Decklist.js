...
import { MTGO, ERROR_DEFAULTS, INTERACTION_RESPONSE_TYPE } from 'constants';
...
const Decklist = {
  ...
  options: [
    {
        name: 'decklist_url',
        description: 'Fetches a decklist from a url to a publically accessible deck page.',
        type: 'string',
    },
    {
        name: 'format',
        description: 'A format name to pull decklists from.',
        type: 'string',
        choices: MTGO.FORMATS
    },
    {
        name: 'archetype',
        description: 'An archetype name (of any format) to pull decklists from.',
        type: 'string'
    },
    /**
     * event-based query
    ...
    {
        name: 'query',
        description: 'Query to filter decklists by.',
        type: 'string'
    },
    {
        name: 'sort_by',
        description: 'Sorting scheme for sorting multiple decklist results.',
        type: 'string',
        choices: [
            { name: 'Publish Date', value: 'deck-date' },
            { name: 'Color', value: 'deck-color' },
            { name: 'Price ($USD)', value: 'deck-price-usd' },
            { name: 'Price (€EUR)', value: 'deck-price-eur' },
            { name: 'Price (TIX)', value: 'deck-price-tix' },
            { name: 'Deck Name', value: 'deck-name' },
            { name: 'Player Name', value: 'player-name' },
        ],
    },
    {
        name: 'ascending',
        description: '(Optional) Toggles whether to sort multiple decklist results in ascending / descending order.',
        type: 'boolean'
    }
  ],
  async execute({ client, interaction, args }) {
    const { decklist_url, archetype, format, query, sort_by, ascending } = args;
    try {
        if (decklist_url) {
            if (interaction) {
