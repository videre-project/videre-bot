...
const Event = {
    ...
    async execute({ client, interaction, args }) {
        ...
        try {
            if (!event_id) return { ...ERROR_DEFAULTS, description: `No \`event_id\` provided.` };
            ...
            `;
            if (!request[0]) return { ...ERROR_DEFAULTS, description: `Event not found.` };

            const date = await sql`SELECT date from events WHERE uid = ${event_id}::int;`;
            ...
            switch (view) {
                ...
                case 0:
                    return formatListAsPages(
                        await Promise.all(request.map( async ({ username, url, deck, stats, archetype }) => (
                            {
                                author: { name: eventName, url: request[0].url.split('#')[0] },
                                title: [
                                    `${getNumberWithOrdinal(stats.rank)} place (${stats.record})`,
                                    `{COLORS} **${archetype[Object.keys(archetype)[0]]?.displayName}** by ${username}`,
                                ].join('\n'),
                                url: url,
                                deck: deck,
